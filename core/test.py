...
from utils import tocuda, get_pool_result, reduce_tensor
...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    with torch.no_grad():
        ...
        for test_data in loader_iter:
            ...
            _, geo_loss, cla_loss, l2_loss, prec, rec = match_loss.run(cur_global_step, test_data, y_hat, e_hat)
    ...
    if mode == "valid":
        for i in range(len(ret_val)):
            ret_val[i] = torch.tensor(ret_val[i],device='cuda:{}'.format(config.local_rank))
            ret_val[i]= reduce_tensor(ret_val[i],'mean')
            ret_val[i] = ret_val[i].cpu().numpy()
